...
var events = d3.dispatch.apply(this,["renderAU", "render", "resize", "highlight", "brush", "brushend", "brushstart", "axesreorder"].concat(d3.keys(__))),
    ...
    // axis = d3.svg.axis().orient("left").ticks(0), // no ticks
...
pc.autoscale = function() {
  ...
  var defaultScales = {
    "date": function(k) {
      ...
      // // Normalized extent by dimension
      // var extent = d3.extent(__.data, function(d) {
      //   return d[k] ? d[k].getTime() : null;
      // });
      // extent is the global extent of all dimensions
      var extent = d3.extent(__.data);
    ...
    "number": function(k) {
      ...
      var extent;
      var extByDim = [];
      var numDim = d3.keys(__.dimensions).length;
      var gMin = 1e10;
      var gMax = -1e10;
      for(var i = 0; i < numDim; i++){
          
        var exxtt = d3.extent(__.data, function(d) {
          var kk = d3.keys(__.dimensions)[i];
          return +d[kk];
        });
        gMin = Math.min(exxtt[0], gMin);
        gMax = Math.max(exxtt[1], gMax);
        // console.log(exxtt);
          extByDim.push(exxtt);
      }
      extent = [gMin, gMax];
      // console.log("extByDim", extByDim);
      //var extent = d3.extent(__.data, function(d) {return d3.extent(d);});
      // console.log("extent", extent);
      // special case if single value
      if (extent[0] === extent[1]) {
        return d3.scale.ordinal()
...
// Draw Angle-uniform
pc.renderAU.default = function() {
  pc.clear('foreground');
  pc.clear('highlight');

  // pc.renderBrushed.default(); // later...

  __.data.forEach(path_foregroundAU);
};
...
function color_pathAU(d, ctx) {
	ctx.beginPath();
		single_angleUni_curve(d, ctx);
	ctx.stroke();
};
...
//==============================================
// Angle-uniform transformations
//==============================================
// Helper functions for spline
/**
...
* Return i such that array[i - 1] <= item < array[i].
*/
function upper_bound(array, item) {
  return binarySearch(array, j => item < j);
}

function poly_func(p, x)
{
	var y = 0.0;
	var deg = p.length - 1;
	for (var i = 0; i < p.length; i++)
	{
		y += p[i] * Math.pow(x, deg);
		deg--;
	}
	return y;
}
...
function spline_interp(x, leftBound, coefs, breaks)
{
	if (x < breaks[0] )
	{
		return poly_func(coefs[0], x - breaks[0]);
	}
	else if (x >= breaks[breaks.length-1])
	{
		return poly_func(coefs[coefs.length-1], x - breaks[breaks.length-1]);
	}
	
	// find the corresponding ctrl points for x
	var it = lower_bound(breaks, x);
	var cid = it - 1;
	var x1 = breaks[cid];
	var y = poly_func(coefs[cid], x - x1);
	return y;
}


// Transformation from traditional parallel coordinates to angle-uniform parallel coordinates
function xform_idxpt_complete_line_desc(a1, a2, a3)
{
  //=========================================================================
  ...
  //=========================================================================
  if (a2 == 0) // for +/- pi/2; the transformed point is at x1 axis
  {
    u = 0.0;
    v = -a3 / a1;
    return { valU: u, valV: v };
  }
  ...
  if (theta > Math.PI / 4) // left of the axes pair
  {
    u = theta * 2 / Math.PI - 1;
...
function equalThetaXformLineTo2D(src, dst, theta0, theta1, subdimId, 
  pi4atleft, num_xf_segments, ctx)
{
	// Get PCP line information
	var dx = dst.x - src.x;
	var P_a = (dst.y - src.y) / dx;
 	src.x -= (subdimId);
	dst.x -= (subdimId);
	var P_b = (dst.x * src.y - src.x * dst.y) / dx;
	// range of theta
	var theta_r = theta1 - theta0;
	// variables for drawing the curve on screen
	var thisScrPt = {x: 0, y: 0};
	var lastScrPt = {x : 0, y: 0};
	//double delta = 0.5 / double(num_xf_segments);
	var delta = 1e-4;
	// For each segment of equal-orientation:
	var u, v;
	var x = 0.0;
	var y = 0.0;
	// Parameters for the indexed point
	var a1 = 0.0;
	var a2 = 0.0;
	var a3 = 0.0;

	// Use the limit approach to solve the case of pi/4
	var limitApproach = true;
	for (var i = 0; i < num_xf_segments; i++)
	{
		var k = i / (num_xf_segments - 1);
		var theta = theta0 + k * theta_r;

		if (Math.abs(theta) == Math.PI / 2)
		{
			// The limit approach
			//limitApproachAtHalfPi(P_a, P_b, u, v)
			// Fixed value at data entry
			a1 = 1.0;
			a2 = 0.0;
			a3 = -P_b;
		}
		else if (theta == Math.PI / 4)
		{
			...
			if (pi4atleft){
				u1 = -0.5 + delta;
				m = Math.tan(0.5 * Math.PI * (u1 + 1)); // left of the left axis
			}
			else{
				u1 = 1.5 - delta;
				m = Math.tan(0.5 * Math.PI * (u1 - 1)); // right of the right axis
			}
			...
			a1 = -m;
			a2 = 1.0; // b of the indexed point's associated line
			a3 = -b;
		}
		else
		{
    ...
    var xf_result = xform_idxpt_complete_line_desc(a1, a2, a3);
    u = xf_result.valU;
    v = xf_result.valV;
		if (theta == Math.PI / 4)
		{
		...
		// The point is always in the LOCAL accum buffer coordinates [-0.5, 1.5]
		//thisPt = Point(u, v);
    // ctx.moveTo();

    // Basically, a move to function
    var thisScrPt = xformedPt2ScreenSpace(u, v, subdimId);
    // console.log("scrPt", thisScrPt);
		if (i > 0){
        ctx.lineTo(thisScrPt.x, thisScrPt.y);
        // lineTo2D(lastScrPt.x, lastScrPt.y, thisScrPt.x, thisScrPt.y, accumBufId);
    }
    else
      ctx.moveTo(thisScrPt.x, thisScrPt.y);
		lastScrPt = thisScrPt;
	}
}
...
function xformedPt2ScreenSpace(u, v, subdimId)
{
  var scrPt = {x: 0, y: 0};
  var p = d3.keys(__.dimensions)[subdimId];
  scrPt.x = (u - BBox.pMin[0]) / (BBox.pMax[0] - BBox.pMin[0]) * __.width;//__.dimensions[p].xscale(u);
  scrPt.y = (1.0 - (v - BBox.pMin[1]) / (BBox.pMax[1] - BBox.pMin[1]))* __.height;//__.dimensions[p].yscale(v);
  return scrPt;
}
...
var Ray = 
{
  ...
  function rayBoxIntersect(box, r) {
    var hit = { t0: 0, t1: 0 };
...
// Draw transformed curve in the angle-uniform space
function single_angleUni_curve(d, ctx) {
  var numSeg = 20;
  var length = d3.keys(__.dimensions).length;
  for(var i = 0; i < length-1; i++)
  {
    var p1 = d3.keys(__.dimensions)[i];
    var p2 = d3.keys(__.dimensions)[i+1];
    var normPt1 = {x: 0, y: d[p1]};
    var normPt2 = {x: 1, y: d[p2]};
    // console.log("NormPt1", normPt1, "NormPt2", normPt2);

    var dir = {x: normPt2.x - normPt1.x, y : normPt2.y - normPt1.y};
    Ray.d[0] = dir.x; Ray.d[1] = dir.y;
    Ray.o[0] = normPt1.x; Ray.o[1] = normPt1.y;
    
    var hit = rayBoxIntersect(BBox, Ray);
    var interPt1 = 
    { x: Ray.o[0] + Ray.d[0] * (hit.t0 - 100.0),
      y: Ray.o[1] + Ray.d[1] * (hit.t0 - 100.0)};

    var interPt2 = 
    { x: Ray.o[0] + Ray.d[0] * (hit.t1 + 100.0),
      y: Ray.o[1] + Ray.d[1] * (hit.t1 + 100.0)};

    //draw seperately in 3 sections: left to the pair, inside the pair, right to the pair
    var pi4atleft = true;
    equalThetaXformLineTo2D(normPt1, interPt1, Math.PI/4, Math.PI/2, i, pi4atleft, 
    numSeg, ctx);
    equalThetaXformLineTo2D(normPt1, normPt2, -Math.PI/2, 0, i, pi4atleft, 
      2 * numSeg, ctx);

    pi4atleft = false;
    equalThetaXformLineTo2D(normPt2, interPt2, 0, Math.PI/4, i, pi4atleft,
      numSeg, ctx);
  }
}
...
function path_foregroundAU(d, i){
  ctx.foreground.strokeStyle = d3.functor(__.color)(d, i);
  
	return color_pathAU(d, ctx.foreground);
...
function flipAxisAndUpdatePCP(dimension) {
  ...
  d3.select(this.parentElement)
    .transition()
      ...
      .call(axis.tickFormat(function(d,i){return 0;}).ticks(__.dimensions[dimension].ticks))
...
pc.createAxesNoTickLabels = function() {
  if (g) pc.removeAxes();
  ...
  g = pc.svg.selectAll(".dimension")
    ...
    .enter().append("svg:g")
      ...
      // .attr("x1", function(d){ 
      //   console.log(__.dimensions[d].position);
      //   return __.dimensions[d].position;})
      // .attr("y1", h()-1)
      // .attr("x2", w())
      // .attr("y2", h()-1)
      // .attr("stroke-width", 3)
  ...
  g.append("svg:g")
      ...
      .each(function(d) {
        ...
        // Remove text
        axisElement.selectAll("text").remove();
